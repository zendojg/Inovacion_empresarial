...
interface FormDao {
    ...
    @Query("SELECT lugarPobNombre FROM DBlugarP_table WHERE lugarPobladoId = :idLugar AND corregimientoId = :idcorre AND  distritoId = :idDist AND provinciaId = :idProv")
    suspend fun getLugarPName(
        idProv: String,
        idDist: String,
        idLugar: String,
        idcorre: String):String
    @Query("SELECT lugarPobNombre FROM DBlugarP_table WHERE  corregimientoId = :idCorre AND  distritoId = :idDist AND provinciaId = :idProv")
    suspend fun getLugarPArrayByID(
        idProv: String,
        idDist: String,
        idCorre: String):Array<String>
    @Query("SELECT lugarPobladoId FROM DBlugarP_table WHERE lugarPobNombre = :lugarname AND corregimientoId = :idcorre AND  distritoId = :idDist AND provinciaId = :idProv")
    suspend fun getLugarPID(
        idProv: String,
        idDist: String,
        idcorre: String,
        lugarname: String):String
